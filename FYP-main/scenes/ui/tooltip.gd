...
var tween: Tween
var is_visible_now := false       # Setting the default as false so that the animation of the card does not play.

func _ready() -> void:
	# The following events have been placed here to so taht when other card states call them, it can either be displayed or not shown.
...
func hide_tooltip() -> void:
	...
	# Setting up a timer for the hide animation.
	get_tree().create_timer(fade_seconds, false).timeout.connect(hide_animation)
	
	# If hide_animation() is missed then it will stick to the code on screen forever. (Temporary)
	#hide_animation()
